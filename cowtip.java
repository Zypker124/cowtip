...
class cowtip {
  ...
  public void run() throws IOException {
	//Use BufferedReader rather than RandomAccessFile; it's much faster
	  ...
	  PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter("cowtip.out")));
	    StringTokenizer st = new StringTokenizer(reader.readLine());
	  
	    int N = Integer.parseInt(st.nextToken());
	    int[][] tippedCows = new int[N][N];
	    for(int ind = 0; ind < N; ind++){
	    	st = new StringTokenizer(reader.readLine());
	    	String line = st.nextToken();
	    	for(int ind2 = 0; ind2 < line.length(); ind2++){
	    		tippedCows[ind][ind2] = Integer.parseInt(line.substring(ind2, ind2+1));
	    	}
	    }
	    
	    int minimumReached = Integer.MAX_VALUE;
	    int count = 0;
	    
	    for(int r = tippedCows.length-1; r>=0; r--){
	    	for(int c = tippedCows[0].length-1; c>=0; c--){
		    	if(tippedCows[r][c]==1){
		    		tippedCows = theSwitch(tippedCows, r+1, c+1);
		    		count++;
		    	}
		    }
	    }
  ...
  public ArrayList<int[][]> generateRects(int[][] tippedCows){
	  ArrayList<int[][]> list = new ArrayList<int[][]>();
	  for(int r = 1; r <= tippedCows.length; r++){
		  for(int c = 1; c <= tippedCows[0].length; c++){
			  list.add(parseRect(tippedCows, r, c));
		  }
	  }
	  return list;
  }
  
  public int[][] parseRect(int[][] tippedCows, int rLength, int cLength){
	  int[][] parsed = new int[rLength][cLength];
	  for(int r = 0; r < rLength; r++){
		  for(int c = 0; c < cLength; c++){
			  parsed[r][c]=tippedCows[r][c];
		  }
	  }
	  return parsed;
  }
  
  public void switchRect(int[][] arr){
  ...
  public int[][] theSwitch(int[][] tippedCows, int rLength, int cLength){
	  for(int r = 0; r < rLength; r++){
		  for(int c = 0; c < cLength; c++){
			  if(tippedCows[r][c]==0){
				  tippedCows[r][c]=1;
  ...
  public boolean checkIfDone(int[][] tippedCows){
	  for(int r = 0; r < tippedCows.length; r++){
		  for(int c = 0; c < tippedCows[0].length; c++){
			  if(tippedCows[r][c]==1){
				  return false;
			  }
		  }
